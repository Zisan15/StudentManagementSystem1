...
//4-DB ile iletişimde olan class: connection, statement, prapered statement
public class StudentRepository {
    ...
    public void createTable() {
        getConnection();
        getStatement();
        try {//id'ye unique constrainti eklenmeli
    ...
    public List<Student> findStudentByNameOrLastname(String nameOrLastname) {
        ...
        getConnection();
        String searced = "%" + nameOrLastname + "%";
        String sql = "SELECT * FROM t_student WHERE name ILIKE ? OR lastname ILIKE ?";
        getPreparedStatement(sql);
        try {
            prst.setString(1, searced);//ILIKE '%nameOrLastname%'
            prst.setString(2, searced);
